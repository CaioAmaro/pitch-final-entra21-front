...
export async function loadHeader() {
    ...
    try {
        ...
        const response = await fetch('/src/components/headerLogado.html'); // AJUSTE ESTE CAMINHO se necessário
        if (!response.ok) {
        ...
        if (searchForm && searchInput) {
            searchForm.addEventListener('submit', (event) => {
                ...
                if (searchTerm) {
                    ...
                    const catalogURL = `/src/pages/catalogo-produtos.html?search=${encodeURIComponent(searchTerm)}`;
